...
export function Step6Result({ data, onBack, onClose }) {
  ...
  const priorities = [
    { id: 1, label: t('symptoms.res_consult'), desc: isHighPriority ? t('symptoms.res_high_prio') : 'Suggested', icon: 'User', color: colors.sCoral },
    { id: 2, label: t('symptoms.res_tested'), desc: isHighPriority ? t('symptoms.res_high_prio') : 'Recommended', icon: 'Activity', color: colors.primary },
    { id: 3, label: t('symptoms.res_cause'), desc: isHighPriority ? t('symptoms.res_high_prio') : 'Optional', icon: 'FileText', color: colors.warning }
  ];
