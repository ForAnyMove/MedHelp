  ...
  return (
    <Screen style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{t('patient_tabs.consultation')} #{id}</Text>
      </View>
      
      <View style={styles.content}>
        <Icon name="Video" size={64} color={styles.iconColor.color} />
        <Text style={styles.message}>
          {t('common.processing_request', 'Opening consultation details...')}
        </Text>
        <Text style={styles.subMessage}>ID: {id}</Text>
        
        <TouchableOpacity 
          style={styles.actionButton}
          onPress={() => router.replace('/home')}
        >
          <Text style={styles.actionText}>{t('patient_tabs.home', 'Go to Dashboard')}</Text>
        </TouchableOpacity>
      </View>
    </Screen>
  );
